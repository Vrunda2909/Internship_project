...
//        Toast.makeText(getApplicationContext(),"uid :"+uid,Toast.LENGTH_LONG).show();
        ...
        list.add(new QuestionModel("Choose from the options below to build a query to select the names of students whose ids are between 13 and 45.(Between)\n" +
                "SELECT id, name\n" +
                "_________ students\n" +
                "_________ id\n" +
                "_________ 13 AND 45;\n",
                ...
                "FROM, WHERE, BETWEEN",
                "FROM, WHERE, BETWEEN"));
        list.add(new QuestionModel("What does the Table Join do?(Join)",
                "Removes one of the tables and splits it to the other",
                "Creates a temporary table with the joined tables data",
                "Removes joined tables ",
                "Creates a temporary table with the joined tables data"));
        list.add(new QuestionModel("Rearrange the code to select student names and all university names(use right join to show all university names).(Right join)\n" +
                "A.\tFROM students\n" +
                "B.\tSELECT students.names, universities.names\n" +
                "C.\tRIGHT OUTER JOIN universities\n" +
                "D.\tON students. University_id = universities.id\n",
                "B, C, D, A",
                "B, A, C, D",
                "A, B, C, D",
                "B, A, C, D"));
        list.add(new QuestionModel("To perform a union operation(Union)",
                "You must use LEFT OUTER JOIN",
                "You must remove all the tables ",
                "Columns in queries must be the same",
                "Columns in queries must be the same"));
        list.add(new QuestionModel("Type in the command to unite the query the results without the duplicates (Union all)",
                "Union",
                "Union all",
                "Unique",
                "Union all"));
                ...
                if(position == list.size()){
                    ...
                    reff.addListenerForSingleValueEvent(new ValueEventListener() {
                        ...
                        public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                            ...
                            if(iquizthree == 0){
                                reff.child("quizthree").setValue(score);
                            }else{
                                if(iquizthree > score){
                                    reff.child("quizthree").setValue(iquizthree);
                                }else{
                                    reff.child("quizthree").setValue(score);
                                }
                            }
                            ...
                            if (iquizvalue == 0){
                                ...
                                if(score > iquizthree){
                                    if(iquizone == 0 && iquiztwo == 0 && iquizfour == 0 && iquizfive ==0){
                                    ...
                                    }else if(iquizone != 0 && iquiztwo == 0 && iquizfour == 0 && iquizfive ==0){
                                        ...
                                        float fquizvalue = (iquizone + iquiztwo + score)/3;
                                        reff.child("quizvalue").setValue(fquizvalue);
                                        ...
                                        float fquizvalue = (iquizone +iquiztwo + score + iquizfour)/4;
                                        reff.child("quizvalue").setValue(fquizvalue);
                                        ...
                                        float fquizvalue = (score + iquizone +iquiztwo +iquizfour +iquizfive)/5;
                                        reff.child("quizvalue").setValue(fquizvalue);
    ...
    public void onBackPressed(){
        new AlertDialog.Builder(QuizThreeActivity.this).setTitle("Exit")
                ...
                .setPositiveButton("Yes", new DialogInterface.OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        SharedPreferences sp = PreferenceManager
                                .getDefaultSharedPreferences(QuizThreeActivity.this);
